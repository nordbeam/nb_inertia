/**
 * Modal Configuration and Type Definitions
 *
 * Central type definitions for the modal system used across all modal components.
 */
 ...
 */
export type ModalPageMetadata = Omit<
  Partial<
    Pick<
      InertiaPage,
      | 'flash'
 ...
 */
export type ModalPrefetchOptions = Omit<
  ModalVisitOptions,
  'modalConfig' | 'loadingComponent' | 'returnUrl' | 'prefetch'
> &
  Partial<PrefetchOptions>;
...
export type ModalLinkVisitOptions = Omit<
  ModalVisitOptions,
  'modalConfig' | 'loadingComponent' | 'returnUrl' | 'prefetch'
>;
...
export interface ModalStackContextValue {
   ...
   * ```
   */
  clearModals: (options?: { fireOnClose?: boolean }) => void;
 ...
 */
export function mergeModalConfig(config?: ModalConfig): ModalConfig {
  return {
 ...
 * ```
 */
export type TypedModalInstance<TProps extends Record<string, unknown> = Record<string, unknown>> =
  Omit<ModalInstance, 'props'> & { props: TProps };
