defmodule NbInertia.Controller do
  ...
  if Code.ensure_loaded?(NbSerializer) do
    ...
    @doc """
    Renders an Inertia response with serialized props.

    This function is only available when `nb_serializer` is installed.

    This combines `assign_serialized_props/2` and rendering.

    ## Parameters

      - `conn` - The connection struct
      - `page_ref` - The atom page reference or component string
      - `props` - A keyword list where values are tuples of `{serializer, data}` or `{serializer, data, options}`
    ...
    ## Examples
        ...
        def show(conn, %{"id" => id}) do
          ...
          render_inertia_serialized(conn, :users_show,
            user: {UserSerializer, user},
            posts: {PostSerializer, posts, lazy: true}
          )
        end
    """
    @spec render_inertia_serialized(Plug.Conn.t(), atom(), keyword()) :: Plug.Conn.t()
    defmacro render_inertia_serialized(conn, page_ref, props \\ []) when is_atom(page_ref) do
      quote bind_quoted: [conn: conn, page_ref: page_ref, props: props] do
        import Inertia.Controller, only: [assign_prop: 3]
        ...
        # Build and serialize props from all shared modules (if they support it)
        shared_module_props =
          Enum.reduce(shared_modules, %{}, fn module, acc ->
            module_props =
              if function_exported?(module, :serialize_props, 2) do
        ...
        if Mix.env() in [:dev, :test] do
          ...
          if MapSet.size(collisions) > 0 do
            ...
            raise ArgumentError,
                  "Prop name collision detected: #{collision_list}"
          end
        end
        ...
        shared_prop_assignments =
          Enum.map(shared_props, fn prop_config ->
            case prop_config do
              %{from: :assigns, name: name, serializer: serializer} ->
                data = Map.get(conn.assigns, name)
        ...
        # Split props into serialized (tuples) and raw (primitives)
        {serialized_props, raw_props} =
          Enum.split_with(props, fn {_key, value} ->
            is_tuple(value) and tuple_size(value) >= 2
          end)

        # Combine shared props with serialized props
        all_serialized_props = shared_prop_assignments ++ serialized_props

        # Render the page
        conn =
          conn
          |> NbInertia.Controller.assign_serialized_props(all_serialized_props)
    ...
    @doc false
    defmacro render_inertia_serialized(_conn, _page_ref, _props \\ []) do
      quote do
        raise """
        NbSerializer package not found

        The function render_inertia_serialized/3 requires the `nb_serializer` package to be installed.
        ...
        Without nb_serializer, use render_inertia with raw data:
